...
import { Vendor, GoldToken } from "../typechain-types";
...
describe("🚩 Challenge 2: 🏵 Token Vendor 🤖", function () {
  ...
  let goldTokenAddress = "";

  let tokenContractArtifact = "";
  if (contractAddress) {
    tokenContractArtifact = `contracts/GoldTokenAutograder.sol:GoldToken`;
  } else {
    tokenContractArtifact = "contracts/GoldToken.sol:GoldToken";
  }

  it("Should deploy GoldToken", async function () {
    const GoldTokenFactory = await ethers.getContractFactory(tokenContractArtifact);

    goldToken = (await GoldTokenFactory.deploy()) as GoldToken;
    goldTokenAddress = await goldToken.getAddress();
  });
  ...
  it("Should deploy Vendor", async function () {
    ...
    vendor = (await VendorFactory.deploy(goldTokenAddress)) as Vendor;

    vendorAddress = await vendor.getAddress();
    console.log("Transferring 1000 tokens to the vendor...");
    await goldToken.transfer(vendorAddress, ethers.parseEther("1000"));
  });
  ...
  describe(" 💵 buyTokens()", function () {
    it("Should let us buy tokens and our balance should go up...", async function () {
      ...
      const startingBalance: bigint = await goldToken.balanceOf(owner.address);
      ...
      const newBalance = await goldToken.balanceOf(owner.address);
  ...
  describe(" 💵 sellTokens()", function () {
    it("Should let us sell tokens and we should get the appropriate amount eth back...", async function () {
      ...
      const startingBalance = await goldToken.balanceOf(owner.address);
      console.log("\t", " ⚖  Starting Token balance: ", ethers.formatEther(startingBalance));

      console.log("\t", " 🙄 Approving...");
      const approveTokensResult = await goldToken.approve(vendorAddress, ethers.parseEther("0.1"));
